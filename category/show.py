def show_create():
    ...
    if search_term:
        df = df[df.apply(lambda row: 
            search_term.lower() in str(row['name']).lower() , axis=1)]
    ...
    if col[0].button("New",type="primary",use_container_width=True):
         st.session_state.page="create_category"
         st.rerun()
    if col[1].button("Edit", type="primary",use_container_width=True):
            ...
            if len(selected_rows) == 0:
                ...
                st.session_state.page = "update_category"
                st.rerun()
    ...
    if col[2].button("Delete", type="primary",use_container_width=True):
            ...
            if len(selected_rows) == 0:
                ...
                st.success(f"{len(selected_rows)} category(s) deleted successfully!")
                st.rerun()
    ...
    with col[3].popover("Export:",use_container_width=True):
        ...
        if st.button("Export as CSV", key="export_csv",use_container_width=True):
            ...
            href = f'<a download="category.csv" href="data:text/csv;base64,{b64}" target="_blank">Click here to download CSV</a>'
        ...
        if st.button("Export as Excel", key="export_excel",use_container_width=True):
            ...
            href = f'<a download="category.xlsx" href="data:{mime_type};base64,{b64}" target="_blank">Click here to download Excel</a>'
            st.markdown(href, unsafe_allow_html=True)