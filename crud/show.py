def show_create():
    ...
    st.title("User List - Data Editor View")
    ...
    if search_term:
        df = df[df.apply(lambda row: 
            ...
            search_term.lower() in str(row['email']).lower(), axis=1)]
    ...
    # Show data editor
    edited_df = st.data_editor(df, hide_index=True, use_container_width=True, key='data_editor')
    ...
    if col[1].button("Edit", type="primary",use_container_width=True):
            ...
            if len(selected_rows) == 0:
                st.warning("Please select a user to edit.")
            elif len(selected_rows) > 1:
                st.warning("Please select only one user.")
            else:
    ...
    if col[2].button("Delete", type="primary",use_container_width=True):
            ...
            if len(selected_rows) == 0:
                ...
                st.success(f"{len(selected_rows)} user(s) deleted successfully!")
                st.rerun()
    ...
    with col[3].popover("Export:",use_container_width=True):
        ...
        if st.button("Export as CSV", key="export_csv",use_container_width=True):
            ...
            href = f'<a download="users.csv" href="data:text/csv;base64,{b64}" target="_blank">Click here to download CSV</a>'
        ...
        if st.button("Export as Excel", key="export_excel",use_container_width=True):
            ...
            href = f'<a download="users.xlsx" href="data:{mime_type};base64,{b64}" target="_blank">Click here to download Excel</a>'
            st.markdown(href, unsafe_allow_html=True)